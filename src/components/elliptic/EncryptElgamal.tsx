...
export const CalAddElliptic = (_xp: string, _yp: string, _xq: string, _yq: string, _p: string) => {
    ...
    if (xr === BigInt(0) && yr === BigInt(0)) {
        //     // debugger;
        console.log({ _xp, _yp, xr, yr, lambda, lam2, _p });
    }
...
export const CalMultiply = (_d: string, _xp: string, _yp: string, _a: string, _p: string) => {
    ...
    const recur: any = ({ x, y, lambda }: { x: bigint, y: bigint, lambda?: bigint }, n: bigint) => {

        console.log({ n });
        if (n === BigInt(0)) {
            return { x: BigInt(0), y: BigInt(0), lambda: BigInt(0) }
        }
        if (n === BigInt(1)) {
            return { x, y }
        }
        if (n % BigInt(2) === BigInt(1)) {
            ...
            if (p2.lambda && p2.lambda === BigInt(0)) return { x, y }
            return CalAddElliptic(x.toString(), y.toString(), p2.x.toString(), p2.y.toString(), _p)
        }
        const temp = CalDoubleElliptic(x.toString(), y.toString(), _a, _p);
        // console.log({ temp })
        return recur(temp, n / BigInt(2))
    }
...
const EncryptElgamal: React.FC = (props: { p?: string, setParams?: (input: any) => any }) => {
    ...
    useEffect(() => {
        for (let i = 1; i <= 10; i++) {
            const mm = (CalMultiply(i.toString(), "4", "1", "1", "7"))
            console.log(mm)
        }
    }, [])
    return (
